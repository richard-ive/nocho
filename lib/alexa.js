...
function Alexa(){
  ...
  this.listen = function(TOKEN){
    ...
    self.audioReq = http2.request(audioOptions, (response) => {
      ...
      streamToBuffer(response, function (err, buffer) {
        ...
        var jsonMessage = JSON.parse(buffer.toString('utf8'));
  ...
  this.ping = function(TOKEN){
    ...
    var pingReq = http2.request(pingOptions, (response) => {

      console.log(`Ping response status ${response.statusCode}`);

      if(response.statusCode === 204){
        setTimeout(self.ping, ((5 * 59) * 1000));
      }else{
